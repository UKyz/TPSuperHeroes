  ...
  const computeSolution = R.curry((maxDistance, solutions) => {
    solutions.forEach(solution => {
      ...
      maxDistance) {
        ...
        const mountsByNow = R.difference(mounts, mountsUsed);
        const mountsOnCityA = R.filter(x => R.equals(x.pos, cityA.name),
          mountsByNow);
        distanceTraveled.push(cityA.name);
        if (mountsOnCityA.length > 0) { // If there are mounts available
          distanceCounter += distance(cityA, cityB) / 4;
          mountsUsed.push(R.head(mountsOnCityA));
        } else {
          distanceCounter += distance(cityA, cityB);
        }
        if (R.not(R.includes(cityA.name, R.slice(0, i, solution.path)))) {
          scoreCounter += cityA.score; // Avoid counting twice the score of a
          // city
        }
        i++;
      }
      solution.distanceTraveled = distanceTraveled;
      solution.score = scoreCounter;
      solution.mountsUsed = mountsUsed;
    });
    return solutions;
  });
